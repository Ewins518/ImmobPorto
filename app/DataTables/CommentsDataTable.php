...
class CommentsDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('email', function ($comment) {
                return '<a href = "mailto: ' . $comment->user->email . '" >' . $comment->user->email . '</a>';
            })
          /*  ->editColumn('approval', function($comment) {
            ...
            ->rawColumns(['email','created_at', 'post', 'action'])
    ...
    public function query(Comment $comment)
    {
        ...
        return $query->with('user:id,name,email,valid', 'post:id,title,slug');
    }
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('body')->title(__('Message')),
